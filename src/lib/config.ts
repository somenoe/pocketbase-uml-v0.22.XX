export const DEFAULT_ICON_SIZE = '1.25em';
export const DEFAULT_ICON_STROKE = 1.75;

const LOCAL_STORAGE_KEY_PREFIX = 'pb-uml';

export const LOCAL_STORAGE_CONNECTION_KEY = `${LOCAL_STORAGE_KEY_PREFIX}.connection`;
export const LOCAL_STORAGE_SETTINGS_KEY = `${LOCAL_STORAGE_KEY_PREFIX}.settings`;

export const MAX_ZOOM = 2;

export const MAX_VISIBLE_NOTIFICATIONS = 5;
export const NOTIFICATION_TIMEOUT = 3000;

export const PUBLIC_URL = 'https://somenoe.github.io/pocketbase-uml-v0.22.XX/';
export const AUTHOR_URL = 'https://github.com/icflorescu';
export const REPO_URL = 'https://github.com/somenoe/pocketbase-uml-v0.22.XX/';
export const SPONSORS_URL = 'https://github.com/sponsors/icflorescu';
